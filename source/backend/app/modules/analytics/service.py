...
class AnalyticsService:
    ...
    async def _verify_course_ownership(self, course_id: uuid.UUID, teacher_id: uuid.UUID) -> None:
    ...
    async def get_dashboard(
        self, course_id: uuid.UUID, teacher_id: uuid.UUID, now: Optional[datetime] = None
    ) -> DashboardResponse:
    ...
    async def get_drop_off_analysis(
        ...
        for lesson in course.lessons:
            ...
            started_students = [sid for sid, slist in student_started_lessons.items() if lesson.id in slist]
            student_count = len(started_students)

            # unique students who stopped at this lesson
            stopped_count = sum(1 for sid, val in student_max_order.items() if val[0] == lesson.id)
            ...
            if student_count >= 30:
                has_warning = drop_off_rate > threshold
                if lesson.type == "video":
                    db_timeline = await self.repository.get_video_timeline_analysis(lesson.id)
                    timeline_analysis = [TimelinePoint(second=sec, count=cnt) for sec, cnt in db_timeline]
            else:
                drop_off_rate = None
        ...
        for module in sorted_modules:
            ...
            for lesson in sorted_lessons:
                l_stats = lesson_data.get(lesson.id, {"drop_off_rate": None, "has_warning": False, "timeline_analysis": None})
                lessons_list.append(
                    LessonDropOff(
                        lesson_id=lesson.id,
    ...
    async def get_lesson_analytics(
        ...
        durations = [act.duration_seconds for act in activities if act.duration_seconds is not None]
    ...
    async def action_recommendation(
        self, course_id: uuid.UUID, lesson_id: uuid.UUID, recommendation_id: uuid.UUID, teacher_id: uuid.UUID, action: str
    ) -> RecommendationActionResponse:
        """Record teacher action (applied/ignored) on an AI Recommendation."""
        await self._verify_course_ownership(course_id, teacher_id)

        rec = await self.repository.get_recommendation_by_id(course_id, lesson_id, recommendation_id)
        if not rec:
            raise AppException(
